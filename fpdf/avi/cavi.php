...
{ 
   ...
   function tblparcim2($titre,$datejour1,$datejour2,$EPH1) 
	...
	function bar10($x,$y,$a,$b,$c,$d,$e,$f,$g,$h,$i,$j,$a1,$b1,$c1,$d1,$e1,$f1,$g1,$h1,$i1,$j1,$cc0,$cc1,$cc2,$cc3,$cc4,$cc5,$cc6,$cc7,$cc8,$cc9,$titre)
    {
	...
	$w=15;
	$this->SetXY($x-20,$y+12);   
	$this->cell($w,-$ap*2,$ap,1,0,'C',1,0);        
	$this->cell($w,-$bp*2,$bp,1,0,'C',1,0);
	$this->cell($w,-$cp*2,$cp,1,0,'C',1,0);
	$this->cell($w,-$dp*2,$dp,1,0,'C',1,0);
	$this->cell($w,-$ep*2,$ep,1,0,'C',1,0);
	$this->cell($w,-$fp*2,$fp,1,0,'C',1,0);        
	$this->cell($w,-$gp*2,$gp,1,0,'C',1,0);
	$this->cell($w,-$hp*2,$hp,1,0,'C',1,0);
	$this->cell($w,-$ip*2,$ip,1,0,'C',1,0);
	$this->cell($w,-$jp*2,$jp,1,0,'C',1,0);
	
	
	$this->SetXY($x-20,$y+12);    
	$this->cell($w,5,$a1,1,0,'C',0,0);
	$this->cell($w,5,$b1,1,0,'C',0,0);
	$this->cell($w,5,$c1,1,0,'C',0,0);
	$this->cell($w,5,$d1,1,0,'C',0,0);
	$this->cell($w,5,$e1,1,0,'C',0,0);
	$this->cell($w,5,$f1,1,0,'C',0,0);
	$this->cell($w,5,$g1,1,0,'C',0,0);
	$this->cell($w,5,$h1,1,0,'C',0,0);
	$this->cell($w,5,$i1,1,0,'C',0,0);
	$this->cell($w,5,$j1,1,0,'C',0,0);
	$this->SetFillColor(230);
	$this->SetXY($x-20,$y+17);    
	$this->cell($w,5,$cc0,1,0,'C',1,0);
	$this->cell($w,5,$cc1,1,0,'C',1,0);
	$this->cell($w,5,$cc2,1,0,'C',1,0);
	$this->cell($w,5,$cc3,1,0,'C',1,0);
	$this->cell($w,5,$cc4,1,0,'C',1,0);
	$this->cell($w,5,$cc5,1,0,'C',1,0);
	$this->cell($w,5,$cc6,1,0,'C',1,0);
	$this->cell($w,5,$cc7,1,0,'C',1,0);
	$this->cell($w,5,$cc8,1,0,'C',1,0);
	$this->cell($w,5,$cc9,1,0,'C',1,0);
	...
	$this->RoundedRect($x-15,$y-108-5, 45, 130, 2, $style = '');
	$this->SetFont('Times', 'B', 11);
    ...
    {	
	    ...
	    $this->mysqlconnect();
		$query="SELECT * FROM calendrier order by jcal asc ";          
		$resultat=mysql_query($query);
		$totalmbr1=mysql_num_rows($resultat);
		$this->SetXY(10,$this->GetY()+20);
		while($row=mysql_fetch_object($resultat))
		{
		$this->cell(30,5,$this->dateUS2FR($this->datePlus($datemep,$row->jcal)),1,0,'L',1,0);$this->cell(40,5,html_entity_decode(utf8_decode($this->nbrtostringcim('avi','produit','id',$row->vaccin,'produit'))),1,0,'L',1,0);
...
function isHoliday($date,$dateDo)
{
	...
	while($row=mysql_fetch_object($resultat))
	{
	$url = explode('-', $this->datePlus($dateDo,$row->jcal));if ($month == $url[1] && $day == $url[2])return html_entity_decode(utf8_decode($this->nbrtostringcim('avi','produit','id',$row->vaccin,'produit')));
	}
	...
	function LineChart($x,$y,$w,$h,$title,$arrData) {
    	...
    	for($ii=0;$ii<count($arrData);$ii++) {
			...
			for($i=0;$i<count($arrData[$ii]['data']);$i++) {
				...
				// draw point
				$this->Circle($xpnt, $ypnt, 0.3, 0, 360, 'DF','',array($arrData[$ii]['color'][0],$arrData[$ii]['color'][1],$arrData[$ii]['color'][2]));
